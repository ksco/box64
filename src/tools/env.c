...
static void applyCustomRules()
{
    ...
    // If dynacache is enabled, the trace code generated by dynarec_trace will
    // be placed into the cache block. If another trace address is switched to,
    // no further trace code will be generated.
    if (box64env.dynarec_trace && (box64env.trace || box64env.trace_init))
        SET_BOX64ENV_IF_EMPTY(dynacache, 0);

    // dynarec_dump will prints dynarec blocks, but the output will be
    // incomplete if cache blocks are used.
...
#define BOOLEAN(NAME, name, default, wine, dynacache)
#define ADDRESS(NAME, name, wine, dynacache)
#define STRING(NAME, name, wine, dynacache) box_free(env->name);
    ENVSUPER()
#undef INTEGER
#undef INTEGER64
#undef BOOLEAN
...
{
#ifdef _WIN32
#define INTEGER(NAME, name, default, min, max, wine, dynacache) box64env.name = wine ? default : 0;
#define INTEGER64(NAME, name, default, wine, dynacache)         box64env.name = wine ? default : 0;
#define BOOLEAN(NAME, name, default, wine, dynacache)           box64env.name = wine ? default : 0;
#define ADDRESS(NAME, name, wine, dynacache)                    box64env.name = 0;
#define STRING(NAME, name, wine, dynacache)                     box64env.name = NULL;
    ENVSUPER()
#undef INTEGER
#undef INTEGER64
#undef BOOLEAN
